...
Distributing a software with external dependencies is slightly more complicated but not uncommon. We will make our project use [postgresql-simple](https://github.com/lpsmith/postgresql-simple/) so we can learn how to deal with dependencies :books:
...
with Powershell you can use this command: `$env:Path += ";C:\Program Files\PostgreSQL\13\bin"`. It will temporary modify the path variable for the current terminal.
...
The idea behind an installer is that you write a script that will explain how to install a software. The syntax is quite low level but it's not that hard either.
...
We can now run the installer and check that our shortcut have been created in the Desktop folder.
...
Indeed, if you try to run the executable, a window should popup with a message like `The code execution cannot be proceed because LIBPQ.dll was not found. Reinstalling the programmay fix the problem.`. Which means it's time to dive into the world of DLLs.

But wait something's strange. When we executed our program earlier, it was working fine. So what happened meanwhile?<br/>
Well the reason it worked before was because we prefixed the command with `stack exec` which made sure the executable would find its required dependencies.
...
Let's cd inside this folder and run our executable without prefixing it with `stack exec`. We should see the same error message poping up.

Time to fix this by making sure our executable knows where to find the DLLs it depends on.

# DLL

`DLL` (dynamic link library) are our runtime dependencies. In our case, because we use `postgresql-simple`, our project requires some postgresql DLLs that we need to find.
...
On the other hand, we see that our executable require 5 postgresql DLL: *libpq*, *libcrypto*, *libiconv*, *libintl* and *libssl*.
...
Well this post was intended to be simple but actually you shouldn't stop here. You should add to your installer an uninstaller which will take care of removing all the folders/files, shortcuts and whatnot.
...
For simple case like this one, writting an installer isn't too hard. If you don't need shortcuts/icons and would satisfy with a simple executable that you can run from a terminal, you might want to investigate:
